...
<html lang="en">
<head>
    <title>Update Supplier Payment</title>
</head>
<body>
<?php
...
$sql = "SELECT * FROM supplierradio WHERE id = $id";
...
if(isset($_POST['submit']))
{
    ...
    $city=($_POST['city']);
    $date=($_POST['date']);
    $sql="UPDATE `supplierradio` SET `s_name`='$s_name',`credit`='$credit',`amount`='$amount',`debit`='$debit',`address`='$address',`phone`='$phone',`cnic`='$cnic',`ntn`='$ntn',`shopName`='$shopName',`city`='$city',`date`='$date'
     where id=$id";
    $result=($con->query($sql));   
    if($result)
    {
        // echo"update successfully";
        header('location:readsupplierradio.php');
    }
    else{
