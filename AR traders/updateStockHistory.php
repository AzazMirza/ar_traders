...
<html lang="en">
<head>
    <title>Update Stock History</title>
</head>
<body>
<?php
...
$sql = "SELECT * FROM stockhistory WHERE id = $id";
...
if(isset($_POST['submit']))
{
    ...
    $sql="UPDATE `stockhistory` SET `Iname`='$Iname',`Quantity`='$Quantity',`rate`='$rate',`value`='$value',`Discount`='$Discount',`perdiscount`='$perdiscount',`Total`='$Total',`Billing`='$Billing',`g_name`='$g_name',`supplier`='$supplier',`vehicalNumber`='$vehicalNumber',`invoice`='$invoice',`date`='$date'
     where id=$id";
    $result=($con->query($sql));   
    if($result)
    {
         header('location:readstockhistory.php');
    }
    else{
