...
<html lang="en">
<head>
    <title>Update Client Payment</title>
</head>
<body>
<?php
...
if(isset($_POST['submit']))
{
    ...
    $city=($_POST['city']);
    $date=($_POST['date']);
    $sql="UPDATE `radio` SET `c_name`='$c_name',`credit`='$credit',`amount`='$amount',`debit`='$debit',`address`='$address',`phone`='$phone',`cnic`='$cnic',`ntn`='$ntn',`shopName`='$shopName',`city`='$city',`date`='$date'
     where id=$id";
    $result=($con->query($sql));   
    if($result)
    {
        // echo"update successfully";
        header('location:readclientradio.php');
    }
    else{
